...
public class StatisticsActivity extends BrainPhaserActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long categoryId = getIntent().getLongExtra(ChallengeActivity.EXTRA_CATEGORY_ID,
        ...
        final boolean isLandscape = deviceOrientation == Configuration.ORIENTATION_LANDSCAPE;
        GridLayoutManager layoutManager = new GridLayoutManager(this, isLandscape ? 3 : 2,
                GridLayoutManager.VERTICAL, false);
        layoutManager.setSpanSizeLookup(new StatisticsSpanSizeLookup(isLandscape));

        recyclerView.setLayoutManager(layoutManager);

        StatisticsAdapter adapter = new StatisticsAdapter(mUserLogicFactory, mChallengeDataSource,
                (BrainPhaserApplication) getApplication(), user, categoryId, isLandscape);
